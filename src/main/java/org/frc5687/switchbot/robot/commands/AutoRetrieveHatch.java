...
public class AutoRetrieveHatch extends CommandGroup {
    public AutoRetrieveHatch(Robot robot) {
        addSequential(new AutoDriveToTarget(robot, .5, 15, .5, "Initial approach"));
        ...
        addSequential(new AutoAlign(robot.getDriveTrain(), robot.getIMU(), -30, 1));
        addSequential(new AutoDriveToTarget(robot, .5, 8, .5, "Deposit"));
